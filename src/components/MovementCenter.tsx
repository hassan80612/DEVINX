...
export function MovementCenter({onNavigate}:{onNavigate?:(target:string)=>void}){
  ...
  async function load(show=true){
    ...
    if(view!=='pending'){
      ...
      const[r1,r2,r3,r4,cats]=await Promise.all([
      ...
      setBills((r1.data||[]) as Bill[]);setRecPays((r2.data||[]) as unknown as RecPay[]);setBillOverrides((r3.data||[]) as BillOverride[]);setPurchases((r4.data||[]) as unknown as Purchase[]);setCustomCategories((cats.data||[]) as CustomCategory[]);
    }
    setLoading(false);
  }
  ...
  return <div className="movementCenter">
    ...
    {searchActive?(
      ...
      <section className="movementGlobalResults">{searchRows.map(row=><article key={row.id} className={'movementGlobalRow '+row.status+' '+(row.sign>0?'incomeRow':'expenseRow')}><div className="movementGlobalMain"><div className="movementGlobalTitle"><span className={'searchStatus '+row.status}>{row.status==='pending'?t('common.pending'):row.status==='planned'?t('move.future'):t('common.history')}</span><b>{row.title}</b></div><small>{row.subtitle} · {date(row.date,{day:'2-digit',month:'short',year:'numeric'})}</small></div><strong className={row.sign>0?'positive':'negative'}>{row.sign>0?'+ ':'− '}{currency(row.amount)}</strong><div className="movementGlobalActions">{row.cash?row.cash.kind==='reserve-transfer'?<button onClick={()=>onNavigate?.('reserves')}>{t('move.openModule')}</button>:row.cash.kind==='tx'&&(row.cash.raw as Tx).source_type==='future_plan'?<button onClick={()=>onNavigate?.('plan')}>{t('future.openPlanning')}</button>:row.cash.kind==='tx'&&(row.cash.raw as Tx).source_type==='card_installment_payment'?<button onClick={()=>onNavigate?.('cards')}>{t('move.openModule')}</button>:<><button onClick={()=>startEdit(row.cash!)}>{t('common.edit')}</button><button className="dangerText" onClick={()=>removeRow(row.cash!)}>{row.cash.kind==='card-payment'?t('move.reopen'):t('common.delete')}</button></>:<button onClick={()=>onNavigate?.(row.target||'movements')}>{t('move.openModule')} <i>›</i></button>}</div></article>)}</section>
    ):view!=='pending'&&<>
