...
export function GoalManager(){
  ...
  function startNew(){setName(goal?.name==='__devinx_default_goal__'?'':goal?.name||'');setTarget(goal?String(Number(goal.target_minor)/100).replace('.',','):'');setPeriod(goal?.period||'monthly');setBasis(goal?.basis||'gross');setOpen(true);setNotice('')}
  ...
  return <div className="goalManager">
    ...
    {!goal?<section className="empty"><b>{t('goals.empty')}</b><p>{t('goals.emptyText')}</p></section>:<section className="goalCard"><div><small>{periodLabel} · {basisLabel}</small><h3>{goal.name==='__devinx_default_goal__'?t('goals.defaultName'):goal.name}</h3></div><strong>{currency(Number(goal.target_minor))}</strong><button className="textButton dangerText" onClick={endGoal}>{t('goals.end')}</button></section>}
  </div>;
}
