...
export function QuickCapture(){
  ...
  async function save(){const value=toMinor(amount);if(value<=0){setMessage('Informe o valor.');amountRef.current?.focus();return}setSaving(true);setMessage('');const supabase=createClient();const{data:{user}}=await supabase.auth.getUser();if(!user){setSaving(false);location.href='/entrar';return}const{error}=await supabase.from('transactions').insert({user_id:user.id,type:mode,category_id:category,description:description.trim()||null,amount_minor:value,occurred_on:localDateISO(),payment_method:mode==='expense'?payment:null,is_avoidable:mode==='expense'?avoidable:false,is_recurring:false});setSaving(false);if(error){setMessage('Não foi possível salvar agora.');return}setAmount('');setAvoidable(false);setMessage(mode==='expense'?'Gasto salvo.':'Entrada salva.');window.dispatchEvent(new CustomEvent('devinx:finance-updated'));setTimeout(()=>{setOpen(false);setMessage('')},550)}
  ...
  return <>
    ...
    {open&&<div className="quickCaptureBackdrop" onMouseDown={e=>{if(e.target===e.currentTarget)setOpen(false)}}><section className="quickCaptureSheet" role="dialog" aria-modal="true" aria-label="Registro rápido">
      ...
      <div className="quickCaptureRoutes"><span>Não achou a categoria?</span><Link href="/categorias" onClick={()=>setOpen(false)}>Gerenciar categorias</Link></div>
