...
const COPY={
  "pt-BR":{
    ...
    chooseText:"Financeiro e Loja são produtos separados.",
    ...
    storeExample:"Ver vitrine de exemplo",
    daily:"Meta diária",
  ...
  es:{
    control:"DEVINX · TÚ TIENES EL CONTROL",choose:"Elige un producto.",chooseText:"Finanzas y Tienda son productos separados.",intlChoose:"Toma el control de tus finanzas.",intlChooseText:"Organiza tu dinero con DevinX Finanzas.",independent:"PRODUCTO INDEPENDIENTE",ownPlan:"Suscripción propia · no incluye el otro producto",or:"O",
  ...
  fr:{
    control:"DEVINX · VOUS GARDEZ LE CONTRÔLE",choose:"Choisissez un produit.",chooseText:"Finance et Boutique sont des produits séparés.",intlChoose:"Prenez le contrôle de vos finances.",intlChooseText:"Organisez votre argent avec DevinX Finance.",independent:"PRODUIT INDÉPENDANT",ownPlan:"Abonnement propre · n’inclut pas l’autre produit",or:"OU",
  ...
  de:{
    control:"DEVINX · DU HAST DIE KONTROLLE",choose:"Wähle ein Produkt.",chooseText:"Finanzen und Shop sind getrennte Produkte.",intlChoose:"Bring deine Finanzen unter Kontrolle.",intlChooseText:"Organisiere dein Geld mit DevinX Finanzen.",independent:"EIGENSTÄNDIGES PRODUKT",ownPlan:"Eigenes Abo · enthält nicht das andere Produkt",or:"ODER",
    ...
    storeTag:"DEVINX SHOP · DEIN GESCHÄFT AN EINEM ORT",storeLine1:"Ein schöner Shop für Kunden.",storeLine2:"Ein organisiertes Geschäft für dich.",storeText:"Shop, Produkte, Bestand, Kosten und Verkäufe an einem Ort — ohne Verkaufsprovision.",storePrimary:"Shop erstellen",storeSecondary:"Innen ansehen",storeLogin:"Ich habe einen Shop · Anmelden",storeExample:"Beispiel-Shop ansehen",
  ...
  ar:{
    control:"DEVINX · أنت المتحكم",choose:"اختر منتجاً واحداً.",chooseText:"المالي والمتجر منتجان منفصلان.",intlChoose:"تحكّم في حياتك المالية.",intlChooseText:"نظّم أموالك مع DevinX المالي.",independent:"منتج مستقل",ownPlan:"اشتراك مستقل · لا يشمل المنتج الآخر",or:"أو",
    ...
    storeTag:"DEVINX المتجر · نشاطك في مكان واحد",storeLine1:"متجر جميل لعميلك.",storeLine2:"ونشاط منظم لك.",storeText:"واجهة المتجر والمنتجات والمخزون والتكاليف والمبيعات في مكان واحد — بدون عمولة على المبيعات.",storePrimary:"أنشئ متجري",storeSecondary:"شاهد من الداخل",storeLogin:"لدي متجر · دخول",storeExample:"عرض متجر تجريبي",
    ...
    <section className={`${styles.products} ${!showStore?styles.financeOnly:""}`} aria-label={intl?c.intlChoose:c.choose}>
      <article className={styles.product}>
        ...
        <div className={styles.preview} aria-hidden="true">
          ...
          <div className={styles.financeHero}>
            <small>{c.daily}</small><strong>{previewMoney(28600)}</strong><span>{locale==="pt-BR"?"até o dia 10":"target example"}</span>
          </div>
          <div className={styles.kpis}>
            <div><small>{c.available}</small><b>{previewMoney(124000)}</b></div>
            <div><small>{c.month}</small><b>{previewMoney(267000)}</b></div>
          </div>
          <div className={styles.progress}><span style={{width:"68%"}}></span></div>
          <div className={styles.financeRows}><span>{locale==="pt-BR"?"Contas":"Bills"}</span><i></i><span>{locale==="pt-BR"?"Cartões":"Cards"}</span><i></i><span>{locale==="pt-BR"?"Reservas":"Savings"}</span></div>
        </div>
      </article>
      ...
      <article className={styles.product}>
        ...
        <div className={styles.copy}>
          ...
          <div className={styles.storeLinks}><a className={styles.loginLink} href="/loja/entrar">{c.storeLogin} →</a><a className={styles.exampleLink} href="/iguassu-shop">{c.storeExample} ↗</a></div>
        </div>
