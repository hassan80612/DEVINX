...
export function RecurringManager(){
  ...
  async function load(){
    ...
    setBills((b||[]) as Bill[]);setPayments((p||[]) as Payment[]);
  }
  useEffect(()=>{load()},[]);
  ...
  async function add(e:FormEvent){
    ...
    if(value<=0){setNotice('Informe um valor maior que zero.');return}
    const s=createClient();
    const{data:{user}}=await s.auth.getUser();
    if(!user)return;
    ...
    if(error){setNotice('Não foi possível salvar a conta.');return}
    setName('');setAmount('');setOpen(false);setNotice('');
  ...
  async function markPaid(e:FormEvent,b:Bill){
    e.preventDefault();
    ...
    const{data:{user}}=await s.auth.getUser();
    if(!user)return;
    const{error}=await s.from('recurring_bill_payments').insert({user_id:user.id,recurring_bill_id:b.id,due_month:localMonthStartISO(),amount_minor:value});
    if(error){setNotice('Não foi possível marcar como paga.');return}
    setPayingId(null);setPaidAmount('');setNotice('Conta marcada como paga.');
    window.dispatchEvent(new CustomEvent('devinx:finance-updated'));
    await load();
  }
  ...
  async function disable(id:string){
    const s=createClient();
    const{error}=await s.from('recurring_bills').update({is_active:false}).eq('id',id);
    if(error){setNotice('Não foi possível desativar a conta.');return}
    window.dispatchEvent(new CustomEvent('devinx:finance-updated'));
    await load();
  }

  return <>
    <button className="primary" onClick={()=>setOpen(v=>!v)}>{open?'Fechar':'+ Nova conta recorrente'}</button>
    {open&&<form className="entryForm" onSubmit={add}>
      ...
      <label>Categoria<select value={category} onChange={e=>setCategory(e.target.value)}><option value="housing">Moradia</option><option value="utilities">Água e energia</option><option value="internet">Internet</option><option value="education">Educação</option><option value="subscriptions">Assinaturas</option><option value="health">Saúde</option><option value="other">Outros</option></select></label>
