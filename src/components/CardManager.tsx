...
export function CardManager({onNavigate}:{onNavigate?:(target:string)=>void}){
  ...
  const[payTarget,setPayTarget]=useState<{card:Card;month:string;amount:number;dueDate:string}|null>(null);const[payDate,setPayDate]=useState(localDateISO());const[saving,setSaving]=useState(false);const[expandedCards,setExpandedCards]=useState<Set<string>>(new Set());const[editingCard,setEditingCard]=useState<Card|null>(null);const[eCardName,setECardName]=useState('');const[eCardLimit,setECardLimit]=useState('');const[eCardClosing,setECardClosing]=useState('');const[eCardDue,setECardDue]=useState('');
