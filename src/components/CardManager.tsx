...
export function CardManager({onNavigate}:{onNavigate?:(target:string)=>void}){
  ...
  const[payTarget,setPayTarget]=useState<{card:Card;month:string;installment:Inst;dueDate:string}|null>(null);const[payDate,setPayDate]=useState(localDateISO());const[saving,setSaving]=useState(false);const[expandedCards,setExpandedCards]=useState<Set<string>>(new Set());const[editingCard,setEditingCard]=useState<Card|null>(null);const[eCardName,setECardName]=useState('');const[eCardLimit,setECardLimit]=useState('');const[eCardClosing,setECardClosing]=useState('');const[eCardDue,setECardDue]=useState('');
  ...
  const stats=useMemo(()=>Object.fromEntries(cards.map(card=>{
    ...
    const statements=[...new Set(unpaid.map(i=>i.billing_month))].sort().map(month=>{
      ...
      return{month,amount:monthRows.reduce((a,b)=>a+Number(b.amount_minor),0),dueDate:monthRows.find(i=>i.due_date)?.due_date||statementDueDate(month,card.due_day),rows:monthRows};
    });
  ...
  async function pay(e:FormEvent){
    e.preventDefault();if(!payTarget)return;setSaving(true);
    ...
    const{data:existing}=await s.from('transactions').select('id').eq('user_id',user.id).eq('source_type','card_installment_payment').eq('source_id',installment.id).limit(1).maybeSingle();
    if(existing?.id){setSaving(false);setPayTarget(null);await load();return}
    const{data:created,error:txError}=await s.from('transactions').insert({
      ...
      description:purchase?.description||payTarget.card.name,amount_minor:Number(installment.amount_minor),
    ...
    const{error:installmentError}=await s.from('card_installments').update({paid_at:paidAt}).eq('id',installment.id).eq('user_id',user.id).is('paid_at',null);
    if(installmentError){
      await s.from('transactions').delete().eq('id',created.id).eq('user_id',user.id);
      setSaving(false);setNotice(t('common.errorSave'));return
    }
    setSaving(false);setNotice(t('cards.itemPaid'));setPayTarget(null);notifyFinanceUpdated();await load();
  }
  ...
  return <div className="cardsPage">
    ...
    <section className="cardList">{cards.length===0?<div className="empty"><b>{t('cards.noCards')}</b><p>{t('cards.noCardsText')}</p></div>:cards.map(card=>{const s=stats[card.id]||{overdue:0,now:0,future:0,total:0,statements:[]};const opened=expandedCards.has(card.id);return <article className={'creditCard collapsibleCard '+(opened?'expanded':'collapsed')} key={card.id}>
      ...
      {opened&&<div className="collapsibleBody">
        ...
        {s.statements.length>0&&<div className="statementList">{s.statements.map(st=><section className="statementGroup" key={st.month}><div className="statementHead"><span>{t('move.dueOn')} {date(st.dueDate,{day:'2-digit',month:'2-digit',year:'numeric'})}</span><b>{currency(st.amount)}</b><small>{t('cards.statementTotal')}</small></div><div className="statementItems">{st.rows.map(item=><button type="button" key={item.id} onClick={()=>{setPayTarget({card,month:st.month,installment:item,dueDate:item.due_date||st.dueDate});setPayDate(localDateISO())}}><span><b>{item.card_purchases?.description||card.name}</b><small>{item.card_purchases?.installment_count&&item.card_purchases.installment_count>1?item.installment_number+'/'+item.card_purchases.installment_count+' · ':''}{t('move.dueOn')} {date(item.due_date||st.dueDate,{day:'2-digit',month:'2-digit'})}</small></span><strong>{currency(Number(item.amount_minor))}</strong><em>{t('cards.payItem')}</em></button>)}</div></section>)}</div>}
      </div>}
    </article>})}</section>
    ...
    {payTarget&&<div className="modalBackdrop" onMouseDown={e=>{if(e.target===e.currentTarget)setPayTarget(null)}}><form className="modalCard" onSubmit={pay}><div className="modalHead"><h2>{t('cards.payItem')}</h2><button type="button" onClick={()=>setPayTarget(null)}>×</button></div><div className="payBillAmount"><small>{payTarget.card.name} · {payTarget.installment.card_purchases?.description||t('cards.purchase')} · {t('move.dueOn')} {date(payTarget.dueDate,{day:'2-digit',month:'2-digit',year:'numeric'})}</small><strong>{currency(Number(payTarget.installment.amount_minor))}</strong><span>{t('cards.individualPaymentHelp')}</span></div><label>{t('cards.payDate')}<input type="date" value={payDate} onChange={e=>setPayDate(e.target.value)} required/></label><div className="modalActions"><button type="button" className="secondary" onClick={()=>setPayTarget(null)}>{t('common.cancel')}</button><button className="primary" disabled={saving} aria-busy={saving}>{saving?<><span className="buttonSpinner"/>{t('common.saving')}</>:t('common.confirm')}</button></div></form></div>}
  </div>;
}
