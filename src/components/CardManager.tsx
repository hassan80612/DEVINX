...
export function CardManager(){
  ...
  async function load(){
    ...
    const[{data:c},{data:i},{data:custom}]=await Promise.all([
    ...
    setCustomCategories((custom||[]) as CustomCategory[]);
  ...
  async function addPurchase(e:FormEvent){e.preventDefault();const value=minor(total);if(value<=0){setNotice('Informe o valor da compra.');return}const s=createClient();const{error}=await s.rpc('create_card_purchase',{p_card_id:cardId,p_category_id:category,p_description:desc.trim()||null,p_total_minor:value,p_purchased_on:purchaseDate,p_installment_count:Number(count),p_is_avoidable:avoidable});if(error){setNotice('Não foi possível salvar a compra.');return}setDesc('');setTotal('');setCount('1');setCategory('other');setAvoidable(false);setPurchaseDate(localDateISO());setOpenPurchase(false);setNotice('Compra registrada e distribuída nas faturas corretas.');window.dispatchEvent(new CustomEvent('devinx:finance-updated'));await load()}
  async function settle(id:string){const s=createClient();const{error}=await s.rpc('settle_card_bill',{p_card_id:id,p_statement_month:localMonthStartISO(),p_paid_on:localDateISO()});if(error){setNotice('Não foi possível marcar a fatura como paga.');return}setNotice('Fatura marcada como paga. O pagamento não virou uma segunda despesa.');window.dispatchEvent(new CustomEvent('devinx:finance-updated'));await load()}

  return <>
    <div className="toolbar"><button className="primary" onClick={()=>setOpenCard(v=>!v)}>+ Adicionar cartão</button>{cards.length>0&&<button className="secondary" onClick={()=>setOpenPurchase(v=>!v)}>+ Registrar compra</button>}<Link className="secondary" href="/categorias">Categorias</Link></div>
