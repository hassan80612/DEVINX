...
export function CardManager({onNavigate}:{onNavigate?:(target:string)=>void}){
  ...
  const stats=useMemo(()=>Object.fromEntries(cards.map(card=>{
    ...
    const rows=inst.filter(i=>i.card_purchases?.card_id===card.id).map(i=>{
      ...
      const paid=i.paid_at&&linkedPaid===0?Number(i.amount_minor):Math.min(Number(i.amount_minor),linkedPaid);
  ...
  async function pay(e:FormEvent){
    ...
    const willFinish=installment.paid+value>=Number(installment.amount_minor);
    if(willFinish){
    ...
    setSaving(false);setNotice(willFinish?t('cards.itemPaid'):t('cards.partialPaid'));setPayTarget(null);setPayAmount('');notifyFinanceUpdated();await load();
  }
