...
export function CardManager({onNavigate}:{onNavigate?:(target:string)=>void}){
  ...
  const[payTarget,setPayTarget]=useState<{card:Card;month:string;amount:number}|null>(null);const[payDate,setPayDate]=useState(localDateISO());const[saving,setSaving]=useState(false);const[expandedCards,setExpandedCards]=useState<Set<string>>(new Set());
  ...
  async function addPurchase(e:FormEvent){
    ...
    if(error){setNotice(t('common.errorSave'));return}setDesc('');setTotal('');setCount('1');setCategory('other');setAvoidable(false);setPurchaseDate(localDateISO());setOpenPurchase(false);setNotice(t('cards.saved'));window.dispatchEvent(new CustomEvent('devinx:finance-updated'));await load();
  }
  ...
  return <div className="cardsPage">
    ...
    <section className="cardList">{cards.length===0?<div className="empty"><b>{t('cards.noCards')}</b><p>{t('cards.noCardsText')}</p></div>:cards.map(card=>{const s=stats[card.id]||{overdue:0,now:0,future:0,total:0,statements:[]};const opened=expandedCards.has(card.id);return <article className={'creditCard collapsibleCard '+(opened?'expanded':'collapsed')} key={card.id}>
      ...
      {opened&&<div className="collapsibleBody">
        {card.limit_minor!=null&&<div className="cardLimitLine"><span>{t('cards.limit')}</span><b>{currency(Number(card.limit_minor))}</b></div>}
        ...
        {s.statements.length>0&&<div className="statementList">{s.statements.map(st=>{const dueDate=statementDueDate(st.month,card.due_day);return <button key={st.month} onClick={()=>{setPayTarget({card,month:st.month,amount:st.amount});setPayDate(localDateISO())}}><span>{t('move.dueOn')} {date(dueDate,{day:'2-digit',month:'2-digit',year:'numeric'})}</span><b>{currency(st.amount)}</b><em>{t('cards.payBill')}</em></button>})}</div>}
      </div>}
    </article>})}</section>

    {payTarget&&<div className="modalBackdrop" onMouseDown={e=>{if(e.target===e.currentTarget)setPayTarget(null)}}><form className="modalCard" onSubmit={pay}><div className="modalHead"><h2>{t('cards.payBill')}</h2><button type="button" onClick={()=>setPayTarget(null)}>×</button></div><div className="payBillAmount"><small>{payTarget.card.name} · {t('move.dueOn')} {date(statementDueDate(payTarget.month,payTarget.card.due_day),{day:'2-digit',month:'2-digit',year:'numeric'})}</small><strong>{currency(payTarget.amount)}</strong></div><label>{t('cards.payDate')}<input type="date" value={payDate} onChange={e=>setPayDate(e.target.value)} required/></label><div className="modalActions"><button type="button" className="secondary" onClick={()=>setPayTarget(null)}>{t('common.cancel')}</button><button className="primary" disabled={saving}>{saving?t('common.saving'):t('common.confirm')}</button></div></form></div>}
  </div>;
}
