...
export function DashboardOverview(){
  ...
  return <div className="dashboardStack">
    <section className="summaryHero premiumSummary">
      <small>{m.dashboard.projected}</small>
      <strong>{brl(numbers.projected)}</strong>
      <span>{m.dashboard.projectedHelp}</span>
    </section>

    <div className="metricGrid dashboardMetrics">
      <article><small>{m.dashboard.income}</small><b>{brl(numbers.income)}</b></article>
      <article><small>{m.dashboard.spent}</small><b>{brl(numbers.expense)}</b></article>
      <article><small>{m.dashboard.pending}</small><b>{brl(numbers.toPay)}</b></article>
      <article><small>{m.dashboard.avoidable}</small><b>{brl(numbers.avoidable)}</b></article>
    </div>

    <section className="panel quickPanel">
      <div className="sectionTitleRow"><div><small>REGISTRO RÁPIDO</small><h2>{m.dashboard.quick}</h2></div></div>
      <div className="actionGrid premiumActions">
        <Link href="/rendas">{m.dashboard.addIncome}</Link>
        <Link href="/gastos">{m.dashboard.addExpense}</Link>
        <Link href="/trabalho">{m.dashboard.addWork}</Link>
      </div>
    </section>
