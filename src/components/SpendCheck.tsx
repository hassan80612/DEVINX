...
export function SpendCheck(){
  ...
  async function load(){
    ...
    const tx=(a.data||[]) as any[],work=(b.data||[]) as any[],billPay=(c.data||[]) as any[],cardPay=(d.data||[]) as any[],bills=(e.data||[]) as RecurringBillLike[],cards=(f.data||[]) as any[],debts=(g.data||[]) as any[],overrides=(i.data||[]) as RecurringOverrideLike[],reserves=(j.data||[]) as any[];
    ...
    const debtPending=debts.reduce((s,x)=>s+Math.min(Number(x.installment_minor||x.outstanding_minor),Number(x.outstanding_minor)),0);
    const today=localDateISO();
    const reserveCoverage=reserves.filter(x=>x.kind==='use'&&x.occurred_on<=today&&x.target_date&&x.target_date>=today).reduce((s,x)=>s+Number(x.amount_minor),0);
    setBalance(income-out);setPending(Math.max(0,billPending+cardPending+debtPending-reserveCoverage));setLoading(false);
  }
