...
export function TransactionManager({kind}:{kind:Kind}){
  ...
  const categories=categoryOptions(kind,customCategories);
  ...
  async function load(){
    ...
    if(!user)return;
    const[{data:rows},{data:custom}]=await Promise.all([
      supabase.from('transactions').select('id,description,amount_minor,occurred_on,category_id,is_avoidable').eq('user_id',user.id).eq('type',kind).order('occurred_on',{ascending:false}).limit(30),
      supabase.from('finance_categories').select('id,kind,name,icon,show_in_quick,is_active').eq('user_id',user.id).eq('kind',kind).order('created_at')
    ]);
  ...
  useEffect(()=>{
    ...
    return()=>{window.removeEventListener('devinx:finance-updated',refresh);window.removeEventListener('devinx:categories-updated',refresh)};
  },[kind]);
  ...
  async function submit(e:FormEvent){
    ...
    const{error}=await supabase.from('transactions').insert({user_id:user.id,type:kind,category_id:category,description:description.trim()||null,amount_minor:amountMinor,occurred_on:date,payment_method:kind==='expense'?payment:null,is_avoidable:kind==='expense'?avoidable:false,is_recurring:false});
    setSaving(false);
  ...
  return <>
    ...
    <div className="toolbar"><button className="primary" onClick={()=>setOpen(v=>!v)}>{open?'Fechar':kind==='income'?'+ Nova entrada':'+ Novo gasto'}</button><Link className="secondary" href="/categorias">Categorias</Link></div>
