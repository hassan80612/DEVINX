...
export default function EntrarPage(){
  ...
  async function enviar(e:FormEvent<HTMLFormElement>){
    ...
    if(modo==='recuperar'){
      ...
      setAviso('Se este e-mail estiver cadastrado, você receberá um link para criar uma nova senha.');
      return;
    }
    ...
    if(modo==='criar'&&!resposta.data.session){
      setAviso('A conta foi criada, mas a confirmação por e-mail ainda está ativa no servidor. Tente entrar em alguns instantes.');
      return;
    }
  ...
  function trocarModo(novo:'entrar'|'criar'|'recuperar'){
    setModo(novo);setAviso('');setSenha('');setConfirmarSenha('');
  }

  const titulo=modo==='entrar'?'Bem-vindo de volta':modo==='criar'?'Crie sua conta':'Recuperar senha';
  const descricao=modo==='entrar'?'Entre para continuar organizando sua vida financeira.':modo==='criar'?'Crie sua conta e entre direto. Sem confirmação por e-mail.':'Informe seu e-mail e enviaremos um link seguro para você criar uma nova senha.';
  ...
  return <main className="authPage">
    ...
    <section className="authCard">
      ...
      <form onSubmit={enviar}>
        ...
        {modo!=='recuperar'&&<label>Senha<input type="password" minLength={6} required value={senha} onChange={e=>setSenha(e.target.value)} placeholder="Mínimo 6 caracteres" autoComplete={modo==='entrar'?'current-password':'new-password'}/></label>}
        {modo==='criar'&&<label>Confirmar senha<input type="password" minLength={6} required value={confirmarSenha} onChange={e=>setConfirmarSenha(e.target.value)} placeholder="Digite a senha novamente" autoComplete="new-password"/></label>}
      ...
      {modo==='entrar'&&<button className="authSwitch" onClick={()=>trocarModo('recuperar')}>Esqueci minha senha</button>}
      <button className="authSwitch" onClick={()=>trocarModo(modo==='criar'?'entrar':modo==='entrar'?'criar':'entrar')}>{modo==='criar'?'Já tenho uma conta':modo==='entrar'?'Ainda não tenho conta':'Voltar para entrar'}</button>
    </section>
  </main>
}
