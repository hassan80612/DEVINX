...
export default function Home(){
  ...
  return <main className="landingV2">
    <header className="top landingTop"><div className="brand"><span className="mark">D</span><b>DEVINX</b></div><div className="landingTopActions"><select className="languageMini" value={locale} onChange={e=>setLocale(e.target.value as any)}>{locales.map(item=><option key={item} value={item}>{languageNames[item]}</option>)}</select><a className="ghost" href="/entrar">{t('landing.login')}</a></div></header>
