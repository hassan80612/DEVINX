...
export default function Onboarding(){
  ...
  const[vehicleType,setVehicleType]=useState<VehicleType>('car');const[vehicleName,setVehicleName]=useState('');const[energy,setEnergy]=useState('gasoline');const[efficiency,setEfficiency]=useState('');const[unitPrice,setUnitPrice]=useState('');const[defaultFuelPct,setDefaultFuelPct]=useState('');
  ...
  async function skipOnboarding(){
    ...
    const{error}=await s.rpc('complete_initial_setup',{p_sources:[],p_vehicle:null});
    if(!error)await s.from('profiles').update({locale}).eq('id',user.id);
    setSaving(false);
    if(error){setNotice(t('onboard.saveError'));return}
    location.href='/painel';
  }

  async function save(includeVehicle:boolean){
    setNotice('');let vehicle:null|{vehicle_type:VehicleType;name:string;energy_type:string;efficiency:number|null;unit_price_minor:number;default_fuel_percent:number|null}=null;
    if(needsWork&&includeVehicle){const actual=vehicleType==='bicycle'?'human':energy;if(actual!=='human'&&(dec(efficiency)<=0||minor(unitPrice)<=0)){setNotice(t('onboard.vehicleError'));return}const pct=defaultFuelPct?dec(defaultFuelPct):null;if(pct!=null&&(pct<=0||pct>100)){setNotice(t('work.defaultFuelPercentInvalid'));return}vehicle={vehicle_type:vehicleType,name:vehicleName.trim()||t('onboard.myVehicle'),energy_type:actual,efficiency:actual==='human'?null:dec(efficiency),unit_price_minor:actual==='human'?0:minor(unitPrice),default_fuel_percent:actual==='human'?null:pct}}
  ...
  return <main className="onboarding">
    <header className="onboardingTop"><div className="brand"><span className="mark">D</span><b>DEVINX</b></div><div className="onboardHeaderRight"><div className="stepDots">{Array.from({length:maxStep},(_,i)=><i key={i} className={i+1<=step?'active':''}/>)}</div><LanguageMenu/></div></header>
    {step===1&&<section className="onboardingCard optionalOnboardingCard"><button className="onboardingSkipX" type="button" onClick={skipOnboarding} disabled={saving} aria-label={t('onboard.skip')}>×</button><small>{t('onboard.initial')} · {t('common.optional')}</small><h1>{t('onboard.moneyFrom')}</h1><p>{t('onboard.moneyHelpOptional')}</p><div className="sourceGrid">{sourceDefs.map(s=><button type="button" key={s.kind} onClick={()=>toggle(s.kind)} className={selected.includes(s.kind)?'selected':''}><span>{s.icon}</span><b>{s.label}</b><small>{selected.includes(s.kind)?t('onboard.selected'):t('onboard.select')}</small></button>)}</div>{selected.includes('other')&&<label>{t('onboard.profession')}<input value={otherName} onChange={e=>setOtherName(e.target.value)}/></label>}</section>}
